...
# YouTube Downloader - VERSIÓN SOLO ARGUMENTOS 5.4
# No usa cookies del navegador
# Añadido soporte para --js-runtimes node
...
# Verificar PATH de yt-dlp y Node.js
check_path() {
    ...
    # Verificar Node.js (necesario para --js-runtimes)
    if ! command -v node &> /dev/null; then
        echo -e "${YELLOW}ADVERTENCIA: Node.js no está instalado${NC}"
        echo "Para que yt-dlp pueda ejecutar código JavaScript, instala Node.js:"
        echo "  curl -fsSL https://deb.nodesource.com/setup_20.x | sudo bash -"
        echo "  sudo apt install -y nodejs"
        echo "Continuando de todas formas, pero puede fallar en algunos videos."
    fi
...
perform_download() {
    ...
    # Opciones para mejorar estabilidad Y RUNTIME JS
    cmd="$cmd --retries 5"
    cmd="$cmd --fragment-retries 5"
    cmd="$cmd --socket-timeout 30"
    cmd="$cmd --js-runtimes node"          # <--- CRUCIAL: usa Node.js para ejecutar JavaScript
    ...
    if [[ $? -eq 0 ]]; then
        ...
        echo "3. Asegúrate de tener Node.js instalado (curl -fsSL https://deb.nodesource.com/setup_20.x | sudo bash - && sudo apt install -y nodejs)"
        echo "4. Prueba sin metadatos: usa -n"
        echo "5. Prueba con otra calidad: -q media"
        return 1
    fi
}
